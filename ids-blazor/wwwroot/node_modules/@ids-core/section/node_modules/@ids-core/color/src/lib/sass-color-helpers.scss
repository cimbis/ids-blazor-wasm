...
@function ch-pow($base, $exponent, $prec: 12) {
  ...
  @if (floor($exponent) != $exponent) {
    ...
    @return ch-nth-root(ch-pow($base, math.div($exponent, $denominator)), math.div($prec2, $denominator), $prec);
  }
...
@function ch-nth-root($num, $n: 2, $prec: 12) {
  ...
  @for $i from 0 through $prec {
    $x: math.div(1, $n) * (($n - 1) * $x + math.div($num, ch-pow($x, $n - 1)));
  }
...
@function ch-color-luminance($color) {
  ...
  @for $i from 1 through 3 {
    ...
    $rgb: if($rgb < 0.03928, math.div($rgb, 12.92), ch-pow(math.div($rgb + 0.055, 1.055), 2.4, 16));
...
@function ch-color-contrast($color1, $color2) {
  ...
  $luminance1: ch-color-luminance($color1) + 0.05;
  $luminance2: ch-color-luminance($color2) + 0.05;
